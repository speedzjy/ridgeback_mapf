...
public:
  ...
  reconfigureCallback(const std::vector<rclcpp::Parameter> &parameters);
...
LaserscanMerger::LaserscanMerger() : Node("laserscan_multi_merger") {
  ...
  this->declare_parameter<std::string>("cloud_destination_topic",
                                       "/merged_cloud");
  ...
  param_callback_handle_ = this->add_on_set_parameters_callback(std::bind(
      &LaserscanMerger::reconfigureCallback, this, std::placeholders::_1));
...
void LaserscanMerger::laserscan_topic_parser() {
  ...
  while (!tokens.empty()) {
    ...
    for (const auto &topic_it : topics) {
      ...
      if (std::find(topic_types.begin(), topic_types.end(),
                    "sensor_msgs/msg/LaserScan") != topic_types.end() &&
  ...
  if ((tmp_input_topics.size() != input_topics.size()) ||
    ...
    if (input_topics.size() > 0) {
      ...
      RCLCPP_INFO(this->get_logger(), "Subscribing to topics\t%ld",
...
void LaserscanMerger::scanCallback(sensor_msgs::msg::LaserScan::SharedPtr scan,
                                   std::string topic) {
